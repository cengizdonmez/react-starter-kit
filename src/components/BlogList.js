      ...
      <h2 className="text-xl">{title}</h2>
       {blogs.map((blog) => (
        <div className="flex-1 truncate py-3 transition-all hover:bg-slate-200" key={blog.id}>
          ...
          <button onClick={() => handleDelete(blog.id)} type="button" className="inline-flex items-center py-1.5 px-3 text-xs font-medium rounded-full text-white bg-red-600 hover:bg-red-700">Delete Post</button>
        </div>
       ))}
    </div>
  );
